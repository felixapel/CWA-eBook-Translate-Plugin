...
- Reject impossible batch/attempt configurations at startup: require
  ...
  `<= BT_REQUEST_MAX_ATTEMPTS` so a max-size request plus one full
  envelope-recovery cycle always fits the attempt budget.
...
- Make the reader prefetch toggle honest (enable rediscovers, disable drops
  ...
  page/language/mode change, use pointer-capture dragging, add a compact
  narrow-viewport bar layout, and fail explicitly on cross-origin reader
  content instead of scanning the parent document.

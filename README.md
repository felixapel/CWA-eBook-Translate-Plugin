...
| `BT_TRUSTED_PROXIES` | (empty) | **Production-safe** rate-limit-key source. Comma-separated CIDRs/IPs of the *peer* (the actual socket source) allowed to set `X-Forwarded-For`. The first XFF hop is used as the rate-limit key only when the peer is in this list; otherwise the peer is used. Example: `127.0.0.1/32,::1/128` for a local nginx, or `10.0.0.0/8` for a private-network reverse proxy. |
